...
import { EtherSymbol, formatEther, getAddress, id } from 'ethers';
...
const WithdrawModal = ({ display, blockInfo, signers, appSettings, provider }) => {
    if (!display.value)
        return null;
    ...
    function withdraw() {
        waitFor(async () => {
            ...
            if (!appSettings.value.relayEndpoint)
                throw 'No Flashbots RPC provided';
            ...
            if ('result' in response && typeof response.result === 'string')
                ...
                throw 'Flashbots RPC returned invalid data';
        });
    }
    function close() {
        batch(() => {
